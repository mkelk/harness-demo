...
`/lists/<id>`, how a task lands in the current list, and deleting a list. For whoever is
about to change the sidebar, a list page, the create-list or delete-list action, the list
name rule, or how the filter bar and the add form behave inside a list. The filter bar
itself (`show`, `q`) is `how/scheduling.md`.
...
flowchart TD
    ...
    Tasks --> Body["h2 list name or All tasks, AddTaskForm (hidden listId),<br/>FilterBar(basePath), TaskList sections"]
...
| The add form's hidden `listId` and the add action's list check | `src/app/components/add-task-form.tsx` → `AddTaskForm`, `AddTaskFormView`; `src/app/actions.ts` → `addTask`            |
...
| Tests                                                          | `src/app/components/lists-nav.test.tsx`, `src/app/components/new-list-form.test.tsx`, `src/lib/lists/validate.test.ts` |
...
| add form      | `<AddTaskForm listId={list?.id} />`: a hidden `listId` input on a list page, none on `/`                                                |
...
| task sections | `<TaskList open done show q today />`, unchanged from `how/tasks.md`                                                                    |
...
## Adding a task to the current list

On `/lists/<id>` the add form carries `<input type="hidden" name="listId" value="<id>">`.
`addTask` in `src/app/actions.ts` reads every list id with `listLists(db)` and calls
`parseTaskInput(formDataToRaw(formData), { listIds })`, so a `listId` that is not an
existing list fails with `List does not exist.` under the form like any other field
error. On `/` there is no `listId` input, which parses to `null` ("No list"). After a
write, `addTask`, `toggleTask` and `removeTask` call `revalidatePath("/")` and
...
| component | `src/app/components/add-task-form.test.tsx` | the hidden `listId` input on a list page and its absence on `/`                                                                                        |
...
| `List does not exist.` under the add form                  | the hidden `listId` names a list deleted since the page rendered                    | reload the page; the sidebar and the hidden input are re-read from the database                         |
