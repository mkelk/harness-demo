...
| Symptom                                                       | Cause                                                                                     | Fix                                                                                                                                  |
| ------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `SQLITE_BUSY: database is locked`                             | two processes write the same file, for example `pnpm dev` and `pnpm test:e2e` on one path | keep `data/dev.db` and `data/e2e.db` separate; stop the other process; never point `DATABASE_PATH` at a file another server has open |
...
| the e2e run sees data from `pnpm dev`, or the other way round | both were pointed at the same file                                                        | e2e uses `data/e2e.db` (`playwright.config.ts`); dev uses `data/dev.db`; reset with `rm -f data/dev.db data/e2e.db`                  |
