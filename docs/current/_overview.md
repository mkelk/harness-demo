...
| [how/scheduling](how/scheduling.md)                         | 2    | Due dates, priority, the overdue mark, the open ordering, and the filter bar (`show`, `q`) on both `/` and `/lists/<id>`                           |
| [how/lists](how/lists.md)                                   | 2    | Lists: the sidebar with open counts and the New list form, `/lists/<id>`, adding a task to the current list, deleting a list                       |
| [data-model](data-model.md)                                 | 3    | The `tasks`, `lists` and `schema_migrations` tables column by column, the `list_id` relation, the index, the ordering rules, the migration ledger  |
...
| `data-model`        | `data-model.md`       | ER view of `tasks`, `lists` and `schema_migrations` with their columns and constraints, including `tasks.list_id` (`ON DELETE SET NULL`) into `lists.id`                                                                                                                                           |
...
| `lists-and-filters` | `how/lists.md`        | the URL (`/` or `/lists/<id>`) into the page, `getList` or `notFound()`, `TaskPage` reading `listLists` and `listTasks({ listId, show, q })` into the sidebar with counts and the sections; `createListAction` with its messages and `deleteListAction` with `ON DELETE SET NULL` and the redirect |
...
**Last documentation update:** 2026-09-17 (epic Lists and filtering, ticks 3e9 and sxg: how/lists added with the `lists-and-filters` diagram, the List select and the row link; tasks and scheduling updated for `TaskPage`, `basePath` and the List field; README updated).
